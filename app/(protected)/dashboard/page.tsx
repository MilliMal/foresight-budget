...
import { getCurrentMonthYear, monthName, monthsUntil } from "@/lib/utils";
          ...
          <div className="card bg-gradient-to-r from-teal-700 to-teal-800 text-white">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{fmt(totalMonthlyExpenses)}</p>
              ...
              <div>
                ...
                <p className="text-2xl font-bold mt-0.5">{fmt(data?.sharedTotal ?? 0)}</p>
              </div>
              <div>
                ...
                <p className={`text-2xl font-bold mt-0.5 ${combinedNet - totalMonthlyExpenses >= 0 ? "text-white" : "text-red-300"}`}>
                  {fmt(combinedNet - totalMonthlyExpenses)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-stone-100 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 bg-stone-50">
                    ...
                    <span className="font-bold text-stone-900">{fmt(totalMonthlyExpenses)}</span>
                  </div>
                  ...
                  {data.sharedDebtPaymentsThisMonth > 0 && (
                    <div className="border-t border-stone-100 pt-2 pb-2">
                      <div className="flex justify-between items-center px-4 py-1 text-sm font-semibold text-stone-700">
                        <span>Shared</span>
                        <span className="text-stone-900">{fmt(data.sharedDebtPaymentsThisMonth)}</span>
                      </div>
                  ...
                  {data.sharedBySection.wedding > 0 && (
                    <div className="grid grid-cols-2 divide-x divide-stone-50 border-t border-stone-50">
                      <div className="px-4 py-2 text-sm">
                        ...
                        <span className="font-medium text-stone-700">{fmt(data.sharedBySection.weddingOneTime)}</span>
                      </div>
                      <div className="px-4 py-2 text-sm">
                        <span className="text-stone-400 text-xs block">Monthly recurring</span>
                        <span className="font-medium text-amber-600">{fmt(data.sharedBySection.weddingMonthly)}/mo</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-xl border border-stone-100 overflow-hidden">
                  ...
                  {data.sharedBySection.son > 0 && (
                    <div className="grid grid-cols-2 divide-x divide-stone-50 border-t border-stone-50">
                      <div className="px-4 py-2 text-sm">
                        <span className="text-stone-400 text-xs block">One-time costs</span>
                        <span className="font-medium text-stone-700">{fmt(data.sharedBySection.sonOneTime)}</span>
                      </div>
                      <div className="px-4 py-2 text-sm">
                        <span className="text-stone-400 text-xs block">Monthly recurring</span>
                        <span className="font-medium text-amber-600">{fmt(data.sharedBySection.sonMonthly)}/mo</span>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {data.sharedBySection.relocation > 0 && (
                    <div className="grid grid-cols-2 divide-x divide-stone-50 border-t border-stone-50">
                      <div className="px-4 py-2 text-sm">
                        ...
                        <span className="font-medium text-stone-700">{fmt(data.sharedBySection.relocationOneTime)}</span>
                      </div>
                      <div className="px-4 py-2 text-sm">
                        <span className="text-stone-400 text-xs block">Monthly recurring</span>
                        <span className="font-medium text-amber-600">{fmt(data.sharedBySection.relocationMonthly)}/mo</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className={`card border-2 ${incomeSurplus !== null && incomeSurplus >= 0 ? "border-teal-200 bg-teal-50" : "border-amber-200 bg-amber-50"}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                <div className="bg-white rounded-lg p-3">
                  ...
                  <p className="font-bold text-stone-900 text-lg mt-0.5">{fmt(weddingRemaining)}</p>
                </div>
                <div className="bg-white rounded-lg p-3">
                  <p className="text-stone-400 text-xs">Monthly savings needed</p>
                  <p className="font-bold text-amber-600 text-lg mt-0.5">{fmt(weddingMonthlyNeeded)}</p>
                ...
                {data.savingsGoals.map(g => (
                  <div key={g.id}>
                    <div className="flex justify-between text-sm mb-1.5">
                      ...
                      <span className="text-stone-500">{fmt(g.alreadySaved)} / {fmt(g.totalTarget)}</span>
                    </div>
